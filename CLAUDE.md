...
4. **`reports/INDEX.md`** — 既存レポート確認（重複調査防止）

> **デフォルトブランチは `master`**（他リポは `main` だが本リポは歴史的経緯で `master`）。URL・コマンドは `master` 基準。
...
- **例外（事前確認必須）**: master への `git push --force`、`gh repo delete`、その他の不可逆・破壊的操作。

---

## 5. ブランチ管理

- **既定: master へ直接コミット**。ブランチ作成はユーザーが明示指示した場合のみ。
- ブランチを作成した場合、回答完了前に必ず `master` へマージ → ブランチ削除 → push まで完了させる。
  - 手順: `git checkout master; git pull; git merge <branch> --no-ff; git push origin master; git branch -d <branch>`
- 「完了 = master にマージ済み＆push済み」。ブランチにファイルを残したまま回答を終えない。
...
3. **報告前に URL 存在確認**: `gh api repos/KazuyaMurayama/academic-research-agent_v1/contents/PATH?ref=master` でステータス200確認。
...
> **レポートの構成・中身のルールは `.claude/rules/output-rules.md`「🎯 レポート構成テンプレート（実践重視）」を必ず参照**。要点：レポートは研究用でなく**ユーザーが実践して成果を出すため**に書く／スコア比較表は序盤（目的説明の直後）／詳細ガイドはA・Bランク（トップ5〜7）のみ・各施策に**具体的なやり方（ステップ・例）**を必須／「エビデンスギャップ・今後の研究課題」セクションは**常に不要**／スコア列名は「効果量・エビデンス強度・**実行性**」。
...
| トリガー | スキル |
|---|---|
| 学術論文・先行研究の調査 | `.claude/skills/research/SKILL.md` |
| 軽量クイックリサーチ | `.claude/skills/research-quick/SKILL.md` |
| キーワード検索のみ | `.claude/skills/search-only/SKILL.md` |
